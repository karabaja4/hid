...
( ffmpeg -f v4l2 -input_format mjpeg -video_size "${_resolution}" -i "${_camera}" -preset ultrafast -vcodec libx264 -tune zerolatency -b:v 3M -maxrate 4M -bufsize 6M -f mpegts "${_address}?listen" & ) > /dev/null 2>&1
